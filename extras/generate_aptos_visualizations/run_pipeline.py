...
def main():
    ...
    # Force clean GPU state before starting (prevents phantom 45GB from crashed sessions)
    import gc
    gc.collect()
    ...
    if torch.cuda.is_available():
        ...
        print(f"GPU memory before start: {allocated:.2f} GB allocated")
        if allocated > 1.0:
            print("WARNING: >1GB already allocated before training starts — consider rebooting for a clean GPU state.")
