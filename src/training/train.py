...
def _compute_loss(outputs, targets, criterion, lambda_qwk, lambda_consistency, label_smoothing=0.0):
    ...
    if 'lam' in targets:
        ...
        return l_ord + (0.3 * l_bin) + (0.2 * l_reg) + (lambda_qwk * l_qwk) + (lambda_consistency * l_cons)
    else:
        # Standard batch
...
def run_curriculum_training(df, img_dir, fold_num, model_name='convnextv2_large', epochs=25, save_dir='results', stabilize=True):
    ...
    if os.path.exists(latest_ckpt):
        if os.path.exists(log_path):
            ...
            if len(df_log) > 0:
                ...
                # If the final logged epoch was a collapse (QWK < 0.05), loading
                # latest.pth would load contaminated EMA weights. Fall back to
                # best_ema.pth instead and roll the log back to the best epoch.
                if last_qwk < 0.05 and os.path.exists(best_ckpt):
                    ...
                    # strict=False: handles BatchNorm1d->LayerNorm transition (BN has extra running_mean/var keys)
                    missing, unexpected = model.load_state_dict(torch.load(best_ckpt, map_location=device), strict=False)
                    if unexpected: print(f"    Skipped checkpoint keys (architecture change): {len(unexpected)}")
                    # Fix: Actually strip the collapsed rows from the log
                    df_log = df_log[df_log['qwk'] > 0.05].reset_index(drop=True)
                else:
                    print(f">>> Persistence: Resuming from {latest_ckpt} <<<")
                    # strict=False: handles BatchNorm1d->LayerNorm transition
                    missing, unexpected = model.load_state_dict(torch.load(latest_ckpt, map_location=device), strict=False)
                    if unexpected: print(f"    Skipped checkpoint keys (architecture change): {len(unexpected)}")
    ...
    if start_epoch > 6 and start_epoch <= 16:  # 768px (10 epochs)
        current_size = 768
        batch_size = 8       # Lower from 14/16 to 8 for stability
        grad_accum = 4       # Increase to 4 to keep effective batch size (8*4=32)
        # FORCE ENABLE CHECKPOINTING (Even for 768px)
        model.backbone.set_grad_checkpointing(enable=True)
    elif start_epoch > 16:  # 1024px (9 epochs)
        current_size = 1024
        batch_size = 8   
        grad_accum = 4   # 4 accum steps: effective BS=32, less VRAM fragmentation per step
    ...
    # Catch up scheduler if resuming
    if start_epoch > 1:
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        lamb_qwk  = 0.0 if epoch <= 15 else 0.03  # Delayed + reduced
        lamb_cons = 0.01 if epoch <= 15 else 0.03  # Reduced by 50%
        ...
        if epoch == 7:  # Transition to Stage 2
            ...
            if 'efficientnet' in model_name:
                ...
                optimizer.param_groups[0]['lr'] = 1.0e-5  # Increased from 8.0e-6
                optimizer.param_groups[1]['lr'] = 3.0e-5  # Increased from 2.5e-5
        ...
        elif epoch == 17:  # Transition to Stage 3 at Epoch 17
            ...
            # HIGHER LR at 1024px
            if 'efficientnet' in model_name:
                optimizer.param_groups[0]['lr'] = 6.0e-6   # Higher than before
                optimizer.param_groups[1]['lr'] = 2.5e-5   # Higher than before
                optimizer.param_groups[2]['lr'] = 2.5e-5
                optimizer.param_groups[3]['lr'] = 2.5e-5
            elif 'convnext' in model_name:
                optimizer.param_groups[0]['lr'] = 1.0e-5  # Much higher
                optimizer.param_groups[1]['lr'] = 3.0e-5  # Much higher
                optimizer.param_groups[2]['lr'] = 3.0e-5
                optimizer.param_groups[3]['lr'] = 3.0e-5
            else:
        ...
        # Latest weights
        torch.save(model.state_dict(), f"{save_dir}/fold_{fold_num}/{model_name}_latest.pth")
